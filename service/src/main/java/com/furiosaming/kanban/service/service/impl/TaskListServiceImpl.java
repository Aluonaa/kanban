...
//    private final UserService userService;
    ...
    public TaskListServiceImpl(TaskListRepository taskListRepository, MemberService memberService, ProjectService projectService) {
...
//        this.userService = userService;
    ...
    public PagingResponseDto<List<TaskListDto>> getAllTaskLists(PagingRequestDto<TaskListFilter> pagingRequestDto) {
        ...
        TaskListFilter taskListFilter = pagingRequestDto.getFilter();
...
//        memberDtoListUserToMemberDto(new ArrayList<>(memberDtoSet), memberDtoListFromUserService);
        ...
        return new PagingResponseDto.Builder<List<TaskListDto>>().success(allTaskListDto, pageRequest.getPageNumber(), pageRequest.getPageSize(), total).build();
                //PagingResponseDto.<List<TaskListDto>>newPageBuilder().ok(Collections.singletonList(allTaskListDto), total, pageRequest.getPageNumber(), pageRequest.getPageSize());
    }


    @Override
    public PagingResponseDto<List<TaskListDto>> allTaskListOfProject(Long id, PagingRequestDto<TaskListFilter> pagingRequestDto) {
        ...
        if (optionalProject.isPresent()) {
            ...
            List<TaskList> allTaskList = pageAllTaskList.getContent();
...
//            List<MemberDto> memberDtoListFromUserService = userService.getUsersFromUserService(userService.getUpnList(memberDtoSet));
//            memberDtoListUserToMemberDto(new ArrayList<>(memberDtoSet), memberDtoListFromUserService);
            return new PagingResponseDto.Builder<List<TaskListDto>>().success(taskListDto, pageRequest.getPageNumber(), pageRequest.getPageSize(), total).build();
        } else {
            return new PagingResponseDto.Builder<List<TaskListDto>>().notFound(ID_PROJECT_NOT_FOUND).build();
        }
    }


    @Override
    public BaseResponseDto<TaskListDto> getByIdTaskList(Long id) {
...
//            memberDtoListUserToMemberDto(memberDto, memberDtoFromUserService);
            ...
            return new BaseResponseDto.Builder<TaskListDto>().success(taskListDto).build();
        } else {
            return new BaseResponseDto.Builder<TaskListDto>().notFound(ID_TASK_LIST_NOT_FOUND).build();
        }
    }

    @Override
    @Transactional
    public BaseResponseDto<TaskListDto> saveTaskList(TaskListDto taskListDto) {
    ...
    private BaseResponseDto<TaskListDto> createTaskList(TaskListDto taskListDto) {
        if (taskListDto.getName() != null && taskListDto.getMaxTask() > 0
                && taskListDto.getAuthor() != null && taskListDto.getProject() != null && taskListDto.getProject().getId() != null) {
            ...
            if (project == null) {
                return new BaseResponseDto.Builder<TaskListDto>().notFound(ID_PROJECT_NOT_FOUND).build();
            }
            if (member == null) {
                return new BaseResponseDto.Builder<TaskListDto>().notFound(ID_AUTHOR_NOT_FOUND).build();
            }
...
//            }
        ...
        return new BaseResponseDto.Builder<TaskListDto>().notFound(MISSING_FIELDS).build();
    }

    private BaseResponseDto<TaskListDto> editTaskList(TaskListDto taskListDto) {
...
//                memberDtoListUserToMemberDto(memberDto, memberDtoFromUserService);
                return new BaseResponseDto.Builder<TaskListDto>().success(taskListDto).build();
            } else return new BaseResponseDto.Builder<TaskListDto>().notFound(MISSING_FIELDS).build();
        }
        return new BaseResponseDto.Builder<TaskListDto>().notFound(ID_TASK_LIST_NOT_FOUND).build();
    }
    ...
    @Override
    @Transactional
    public BaseResponseDto<TaskListDto> deleteTaskList(Long id) {
